...
def write_page_url(driver, product_url_txt):        #写入url(OK)
    product_list = driver.find_elements_by_xpath("//a[@class='_3x-5VWa']")
...
def crawl(page_url):
  ...
  # _1QmxDO_ YlqXo8e
    write_page_url(driver, product_url_txt)  #写入url
    
    page_number = 1
    while 1:
        page_string = '&pge=' + str(page_number) + '&pgesize=72'
        driver.get(page_url + page_string)
        
        if check_exists_by_xpath(driver, "//a[@class='3x-5VWa']"):
...
product_url_txt = open('D:/product_url.txt', 'a')
file = open("D:/saos_url.txt")
